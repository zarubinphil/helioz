...
你睡觉的时候活不会停：Helioz 继续往前推，早上告诉你动了什么。
...
Helioz 是一条传送带。你只说一句话：要做什么。它一直追问，直到目标清楚，然后把目标切成小任务分给智能体。干活的那个从不检查自己的活。这些都在你自己的机器上跑，它知道的一切都写在磁盘上。

## 它解决什么问题

合上笔记本，智能体就忘了任务。没做完也说做完了。为一点小事半夜叫醒你。这里每一条毛病都对应一件工具，而不是一句承诺：状态写在磁盘，检查的人不是干活的人，夜里的小岔路交给议事会。早上你读到的是进展，而不是一个停住的对话框。

## 最大的优势

**最大的优势：** 完成标记由代码写下，不是由干活的那个智能体写。

**为什么这样更好：** 标记里有前后两个提交、改动文件的哈希、检查命令的退出码。伪造行不通。从别的任务复制来的、或者手敲上去的标记，会被抓出来并当场点名。

## 工作流程

节拍绕圈跑，每一步都在磁盘上留下文件。半路把会话杀掉，下一个会话从同一行接着走。
...
需要 Mac 或 Linux、Node.js 20 以上、git，以及终端里至少一个智能体：`claude`、`codex` 或 `kimi`。从这里有三扇门，走哪扇都行。
...
**你会得到：** 安装脚本先说明自己是什么，再看你已经有什么，跑一遍自检，最后老实告诉你还剩什么要自己做。
...
任何 push 之前，先看 `git diff`，再跑一遍公开闸门。
...
想更深：[完整参考](docs/DETAILS.md)、[状态契约](docs/CONTRACTS.md)、[编排提示词](ORCHESTRATOR.md)、[总计划](docs/MASTER-PLAN.md)。不想只听说法？`node scripts/helioz-probes.mjs` 会跑十三个专门骗它的探针，必须全绿。
