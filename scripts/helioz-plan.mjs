...
//
// Допрос собран из проверенного и доведен до уровня Fable 5:
...
//                               решает модель. Отсюда измеримое покрытие допроса вместо ощущения «вроде все»;
//   · Elicitron (Ataei et al.) - латентные требования: владелец не назовет то, о чем не думал; спрашивать
...
//   answer --slot X --text "…"   записать ответ владельца в BRIEF (зовет Зевс, когда владелец пишет)
...
export const ONTOLOGY = [
  ...
  { block: 'ГРАНИЦЫ', slots: [
    ...
    { id: 'limits.foreign', critical: false, what: 'чья еще работа идет рядом и где риск столкнуться' },
  ] },
  { block: 'РИСК', slots: [
  ...
  { block: 'УЖЕ ЕСТЬ', slots: [
    ...
    { id: 'have.gaps', critical: false, what: 'чего точно нет и придется делать с нуля' },
  ] },
  { block: 'РАЗВИЛКИ', slots: [
    { id: 'fork.autonomy', critical: true, what: 'что агенты решают сами, а что обязаны вынести владельцу' },
    { id: 'fork.style', critical: false, what: 'предпочтения владельца, которые он не назовет сам (стек, стиль, привычки)' },
  ] },
  // Латентный блок (Elicitron): владелец не назовет то, о чем не думал. Спрашиваем прицельно.
  { block: 'СКРЫТОЕ', slots: [
...
const grillPrompt = idea => [
  'Ты - дознаватель. Владелец принес работу, и твоя задача - вытащить из него все, что нужно, ДО того как',
  'кто-то напишет строку кода. Ты не советчик и не рецензент: ты задаешь вопросы.',
  '',
  `ИДЕЯ ВЛАДЕЛЬЦА: «${idea}»`,
  '',
  ...
  '   вместо вопроса напиши, что нашел, а спроси уже про следующий уровень решения. Не заставляй владельца',
  '   искать то, что видно тебе.',
  '5. Где выбор - предложи 2–3 конкретных варианта прямо в вопросе, а не открытое поле.',
  '6. Блок СКРЫТОЕ - про то, о чем владелец сам не подумает: крайний случай, срок жизни решения, ловушка',
  ...
  '**Ответ:**',
  '',
  '…и так по каждому слоту, в порядке списка выше. Строка «**Ответ:**» пустая - ее заполняет владелец.',
].join('\n')
...
const PLAN_SCHEMA = `{
  ...
  "tasks": [{
    ...
    "verifier_prompt": "дословный промт проверяющему: что проверить по ДИСКУ, первая строка ответа - ВЕРДИКТ: зеленый|красный"
  }]
}`
...
const planPrompt = (goal, brief) => [
  ...
  'дроби. Двенадцать мелких задач лучше трех больших.',
  '',
  'Жесткие требования к каждой задаче:',
  '- check_cmd обязателен и дает 0 только при настоящем успехе. Задача без него в конвейер не принимается.',
  '- probe_cmd - враждебная проба: специально сломанный вход, на котором проверка ОБЯЗАНА покраснеть.',
  '- paths - точные пути, не больше трех на задачу; задачи одной волны не делят ни одного пути.',
  ...
  '  запрещено читать отчет исполнителя - он смотрит только на диск и коды возврата.',
...
const synthPrompt = (goal, variants) => [
  ...
  'Собери ОДИН план: бери лучшее из каждого, дроби все, что осталось крупным, выкидывай дубли.',
...
function writeMaster(plan, meta) {
  ...
  writeFileSync(MASTER, [
    ...
    '---', '',
    `Собран прибором helioz-plan ${now()}: планировали ${meta.planners.join(' и ')} независимо, свел ${meta.synth} вслепую.`,
...
function cmdPlan() {
  ...
  if (errs.length) {
    console.error('ПЛАН НЕ ПРИНЯТ (ворота приемки):')
    for (const e of errs) console.error('  · ' + e)
    return 1
  }
  const ids = writeTaskFiles(plan)
  writeMaster(plan, { planners, synth: usedSynth })
  zeus(`🗺 Планы готовы: ${ids.length} малых задач и мастер-план. Планировали независимо ${planners.join(' и ')}, свел вслепую ${usedSynth}. У каждой задачи своя команда проверки и враждебная проба. Начинаю работу.`)
...
async function cmdSelftest() {
  ...
  const brief = ['<!-- slot:goal.done -->', '**Вопрос:** Что готово?', '**Рекомендую:** сборка зеленая', '**Ответ:** тесты проходят',
  ...
  try {
    ...
    writeFileSync(stub, `#!/bin/sh\ninput=$(cat)\ncase "$input" in\n  *"дознаватель"*) ${slotBlocks};;\n  *"КОНЕЧНУЮ ЦЕЛЬ"*) echo "# Конечная цель владельца"; echo "Готово, когда: команда проверки дает ноль."; echo "Нельзя: трогать прод.";;\n  *) echo '${JSON.stringify(good).replace(/'/g, "'\\''")}';;\nesac\n`)
    ...
    eq(run(['coverage']).status, 0, 'критичные закрыты - покрытие зеленое')
    ...
    eq((ready.invalid || []).length, 0, 'гейт принял все рожденные задачи')
