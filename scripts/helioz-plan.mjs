...
function cmdGrill(idea) {
  ...
  writeFileSync(BRIEF, [
    ...
    '> Заполняй поля «**Ответ:**» — прямо здесь или сообщениями боту, он допишет сам.',
  ...
  const cov = coverage()
  zeus(`📋 Допрос по идее «${idea.trim().slice(0, 100)}» готов: ${cov.total} вопросов, из них ${cov.criticalTotal} критичных. Отвечай сюда по одному — я записываю сам. Или заполни queue/BRIEF.md руками.`)
  cmdAskNext()
...
function cmdAskNext() {
  ...
  zeus([
    `❓ Вопрос ${c.answered + 1} из ${c.total}${c.next.critical ? ' (критичный — без него планов не будет)' : ''}`,
    c.next.question, '',
    c.next.recommend ? `Моя рекомендация: ${c.next.recommend}` : '',
    'Ответь сообщением. Согласен с рекомендацией — напиши «да».',
  ].filter(Boolean).join('\n'))
  console.log(`спрошен слот ${c.next.id}`)
  return 0
}
