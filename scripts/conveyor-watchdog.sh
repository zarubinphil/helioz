...
HOME_DIR="${CONVEYOR_HOME:-$(dirname "$SELF")}"
STATE="$HOME_DIR/.conveyor/state"
LOGS="$STATE/logs"
mkdir -p "$STATE" "$LOGS"

cfg() { node -e "const c=require('$HOME_DIR/config/conveyor.json');console.log(c.$1 ?? '')" 2>/dev/null; }
...
decide() {
  ...
  local pending
  pending=$(node "$SELF/conveyor-gate.mjs" --status --json 2>/dev/null | node -e 'let d="";process.stdin.on("data",c=>d+=c);process.stdin.on("end",()=>{try{const j=JSON.parse(d);console.log(j.tasks.filter(t=>t.state==="pending"||t.state==="running").length)}catch{console.log(0)}})' 2>/dev/null || echo 0)
...
if [ "${1:-}" = "--selftest" ]; then
  ...
  mkdir -p "$TMP/.conveyor/state" "$TMP/queue/tasks" "$TMP/config" "$TMP/scripts"
  cp "$SELF/conveyor-gate.mjs" "$TMP/scripts/"
  echo '{"watchdog_stale_min":15}' > "$TMP/config/conveyor.json"
  # проба 1: STOP → SKIP
  date > "$TMP/.conveyor/state/STOP"
  out=$(CONVEYOR_HOME="$TMP" bash "$0" --dry-run)
  case "$out" in SKIP:STOP*) ;; *) echo "FAIL: при STOP получили '$out'"; exit 1;; esac
  rm "$TMP/.conveyor/state/STOP"
  # проба 2: свежий heartbeat → SKIP
  node -e "require('fs').writeFileSync('$TMP/.conveyor/state/heartbeat.json',JSON.stringify({at:new Date().toISOString()}))"
  out=$(CONVEYOR_HOME="$TMP" bash "$0" --dry-run)
  ...
  node -e "require('fs').writeFileSync('$TMP/.conveyor/state/heartbeat.json',JSON.stringify({at:new Date(Date.now()-3600e3).toISOString()}))"
...
node "$SELF/conveyor-zeus.mjs" poll --timeout 20 >> "$LOGS/watchdog.log" 2>&1 || true
node "$SELF/conveyor-zeus.mjs" flush >> "$LOGS/watchdog.log" 2>&1 || true
...
node "$SELF/conveyor-zeus.mjs" send --text "♻️ Оркестратор перезапущен сторожем (heartbeat протух). Продолжаю с хэндоффа, состояние цело." >> "$LOGS/watchdog.log" 2>&1 || true
